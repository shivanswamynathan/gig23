...
class SmartFileClassifier:
    ...
    Updated to include better image file support
    """
    ...
    def download_and_analyze(url: str) -> Dict[str, Any]:
        ...
        try:
            # Step 1: Download file (URL-encode the path for S3 keys with special characters)
            parsed = urlparse(url)
            encoded_path = quote(parsed.path)
            encoded_url = urlunparse((parsed.scheme, parsed.netloc, encoded_path, '', '', ''))
            logger.info(f"Downloading file from: {encoded_url}")
